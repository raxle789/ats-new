...
export async function getCandidateExperiences(): Promise<TypeFetchReturned> {
  ...
  try {
    const [experiences, expectedSalary] = await prisma.$transaction([
      prisma.working_experiences.findMany({ where: { id_of_candidate: authSession.candidate.id } }),
      prisma.candidates.findUnique({ where: { id: authSession.candidate.id }, select: { expected_salary: true } })
    ]);
    return {
      success: true,
      data: {
...
export async function getEducationSkills(): Promise<TypeFetchReturned> {
  const authSession = await getUserSession('auth');
  try {
    const [education, skills, languages] = await prisma.$transaction([
      prisma.educations.findUnique({ where: { id_of_candidate: authSession.candidate.id } }),
      prisma.candidateSkills.findMany({ where: { id_of_candidate: authSession.candidate.id }, include: { skills: { select: { name: true } } } }),
      prisma.languages.findMany({ where: { id_of_candidate: authSession.candidate.id }, select: { name: true, proficiency: true } })
    ]);
...
export async function getAdditionalInformations(): Promise<TypeFetchReturned> {
  const authSession = await getUserSession('auth');
  try {
    const [families, emergencyContact, candidateQuestions] = await prisma.$transaction([
      prisma.families.findMany({ where: { id_of_candidate: authSession.candidate.id } }),
      prisma.candidates.findFirst({ where: { id: authSession.candidate.id }, select: { emergency_contacts: true } }),
      prisma.candidateQuestions.findMany({ where: { candidateId: authSession.candidate.id }, select: { questionId: true, answer: true } })
    ]);
    return {
      success: true,
      data: {
