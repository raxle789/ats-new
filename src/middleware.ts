...
export async function middleware(request: NextRequest) {
  ...
  const GETROLES = async (authToken: string) => {
    try {
      const decoded = await jwtVerify(authToken, new Uint8Array(Buffer.from('sidokaredev24')), { algorithms: ['HS256'] });
  ...
  const _ROLES = await GETROLES(_COOKIES.get(authSession)?.value as string) as string[];
  const listOfRole = {
