...
export async function getAllFpk(offset, perPage) {
  try {
    ...
    const [data, total] = await prisma.$transaction([
      ...
      prisma.$queryRaw`SELECT COUNT(*) AS total FROM MASTER_ERA.dbo.PROINT_trx_efpk WHERE Status = 'Approved'`,
    ]);
...
export async function getAllFpkTotal() {
  try {
    ...
    const total = await prisma.$queryRaw`SELECT COUNT(*) AS total FROM efpk`;
